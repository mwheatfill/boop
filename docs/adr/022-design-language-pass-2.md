...
| Token architecture | **Three anchors + derivation.** `--theme-base` (background hue), `--theme-accent` (primary action color), `--theme-contrast` (0..1 multiplier for foreground intensity). Every other token derives via OKLCH relative-color syntax `oklch(from <color> <l> <c> <h>)`. | The per-token block from pass 1 (~40 hardcoded values per mode). Linear themselves moved from 98 per-theme variables to 3 because the curated approach drifts under hand-tuning. |
