...
| Provider credentials | **One provider Cloudflare API token in Workers Secrets Store** (alongside `BOOP_SECRETS_KEK`), with account ID and wildcard zone as config. Not in `workspace_secrets` (it is instance-level, not per-Workspace user data). | Per-Workspace storage (wrong scope for a single provider credential); env-only token (no rotation surface). |
...
- Model B concentrates trust: one provider API token can manage tunnels, Access apps, and DNS across the wildcard zone. It lives in Workers Secrets Store with a rotation surface, but its blast radius is the whole provider account.
