...
| Form composition | shadcn `Field` + `Select` / `Combobox` / `InputGroup` | ADR-023's modal-with-inline-pills; hand-rolled pill inputs |
...
**Supersession.** Supersedes ADR-022's theme *values* (accent, brand split, radius, fonts) and ADR-023 (modal-with-pills) in full. Retains ADR-022's flat-token contract and light-first CSS / dark-default UX structure.
...
2. **Kill the pills**: rebuild the six entity modals (Job, Target, Channel, AlertRule, Tunnel, Workspace) on `Field` + `Select` / `Combobox`; delete the retired primitives.
