...
Install the latest stable major. Don't pin to whatever version you remember from training. Don't pin to an older major without a documented compatibility reason.

When pinning to a non-current version, record why in `package.json` with a JSON comment file or in a single-line note in the corresponding ADR or in `agent-rules/architecture.md`.
