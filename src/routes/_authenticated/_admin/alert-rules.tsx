import { useSuspenseQuery } from '@tanstack/react-query'
import { createFileRoute, Link, Outlet } from '@tanstack/react-router'
import type { ColumnDef } from '@tanstack/react-table'
import { Bell, Plus } from 'lucide-react'
import { z } from 'zod'
import { DataTable } from '@/components/DataTable'
import { EmptyState } from '@/components/EmptyState'
import { StatusBadge } from '@/components/StatusBadge'
import { Button } from '@/components/ui/button'
import { workspaceAlertRulesQueryOptions } from '@/lib/alert-rules/query-options'
import { workspaceChannelsQueryOptions } from '@/lib/channels/query-options'
import { type AlertRule, summarizeRuleConfig } from '@/shared/schemas/alert-rule'

const searchSchema = z.object({ archived: z.boolean().optional() })

export const Route = createFileRoute('/_authenticated/_admin/alert-rules')({
  validateSearch: searchSchema,
  loaderDeps: ({ search }) => ({ archived: Boolean(search.archived) }),
  loader: async ({ context, deps }) => {
    await Promise.all([
      context.queryClient.ensureQueryData(workspaceAlertRulesQueryOptions(deps.archived)),
      context.queryClient.ensureQueryData(workspaceChannelsQueryOptions(true)),
    ])
  },
  component: WorkspaceAlertRulesPage,
})

function columnsFor(channelNameById: Map<string, string>): ColumnDef<AlertRule>[] {
  return [
    {
      accessorKey: 'name',
      header: 'Name',
      cell: ({ row }) => (
        <Link
          to="/alert-rules/$ruleSlug"
          params={{ ruleSlug: row.original.slug }}
          className="font-medium text-foreground hover:underline"
        >
          {row.original.name}
        </Link>
      ),
    },
    {
      accessorKey: 'kind',
      header: 'Predicate',
      cell: ({ row }) => summarizeRuleConfig(row.original.config),
    },
    {
      accessorKey: 'channelIds',
      header: 'Channels',
      cell: ({ row }) =>
        row.original.channelIds.map((id) => channelNameById.get(id) ?? id).join(', '),
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ row }) => <StatusBadge status={row.original.status} />,
    },
  ]
}

function WorkspaceAlertRulesPage() {
  const search = Route.useSearch()
  const archived = Boolean(search.archived)
  const navigate = Route.useNavigate()
  const { data: rules } = useSuspenseQuery(workspaceAlertRulesQueryOptions(archived))
  const { data: channels } = useSuspenseQuery(workspaceChannelsQueryOptions(true))
  const channelNameById = new Map(channels.map((c) => [c.id, c.name]))

  return (
    <div className="flex flex-col gap-6">
      <header className="flex items-start justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
            Alert Rules
          </p>
          <h1 className="text-2xl font-semibold tracking-tight">Workspace routing</h1>
          <p className="text-sm text-muted-foreground">
            Predicates that fire across every Customer when a Run becomes terminal.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate({ search: archived ? {} : { archived: true } })}
          >
            {archived ? 'Hide archived' : 'Show archived'}
          </Button>
          <Button size="sm" render={<Link to="/alert-rules/new" />}>
            <Plus aria-hidden /> New Alert Rule
          </Button>
        </div>
      </header>

      {rules.length === 0 ? (
        <EmptyState
          icon={Bell}
          title={archived ? 'No archived workspace rules.' : 'No workspace alert rules yet.'}
          description={
            archived
              ? 'Toggle off "Show archived" to see active rules.'
              : 'Wire a workspace predicate to one or more workspace Channels.'
          }
          action={
            !archived ? (
              <Button render={<Link to="/alert-rules/new" />}>
                <Plus aria-hidden /> New Alert Rule
              </Button>
            ) : undefined
          }
        />
      ) : (
        <DataTable columns={columnsFor(channelNameById)} data={rules} />
      )}
      <Outlet />
    </div>
  )
}
