...
import { workspaceRuleCountQueryOptions } from '@/lib/alert-rules/query-options'

interface AlertsAppliedPanelProps {
  workspaceSlug: string
}

export function AlertsAppliedPanel({ workspaceSlug }: AlertsAppliedPanelProps) {
  const { data } = useQuery(workspaceRuleCountQueryOptions(workspaceSlug))
      ...
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm">
          <span className="font-medium">
            {count} Workspace-level rule{count === 1 ? '' : 's'} apply
          </span>{' '}
