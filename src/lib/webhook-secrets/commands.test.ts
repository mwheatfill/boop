...
import { decryptSecret, generateKekBase64 } from '@/lib/workspace-secrets/envelope'
...
describe('generateSecret', () => {
  it('returns a 64-hex plaintext but stores ciphertext + iv, not the raw key', async () => {
    const { db, jobId } = await seedJob()
    const result = await generateSecret({ db, kek, now: () => FIXED }, jobId)
    ...
    expect(row.secret).not.toBe(result.plaintext)
    expect(await decryptSecret(row.secret, row.secretIv, kek)).toBe(result.plaintext)
  })

  it('produces a fresh plaintext on every call', async () => {
    const { db, jobId } = await seedJob()
    const [first, second] = await Promise.all([
      generateSecret({ db, kek, now: () => FIXED }, jobId),
      generateSecret({ db, kek, now: () => new Date(FIXED.getTime() + 1) }, jobId),
    ])
...
describe('rotateSecret', () => {
  it('writes a new active secret and sets expires_at on every previously-active secret', async () => {
    const { db, jobId } = await seedJob()
    const first = await generateSecret({ db, kek, now: () => FIXED }, jobId)
    const rotateAt = new Date(FIXED.getTime() + 60_000)
    const second = await rotateSecret({ db, kek, now: () => rotateAt }, jobId, { overlapHours: 24 })
  ...
  it('does not touch secrets that already have an expires_at (mid-rotation)', async () => {
    const { db, jobId } = await seedJob()
    await generateSecret({ db, kek, now: () => FIXED }, jobId)
    const firstRotateAt = new Date(FIXED.getTime() + 60_000)
    await rotateSecret({ db, kek, now: () => firstRotateAt }, jobId, { overlapHours: 24 })

    const secondRotateAt = new Date(firstRotateAt.getTime() + 60_000)
    await rotateSecret({ db, kek, now: () => secondRotateAt }, jobId, { overlapHours: 12 })
  ...
  it.each([0, -1, 169, Number.NaN])('rejects overlapHours = %s', async (overlap) => {
    const { db, jobId } = await seedJob()
    await expect(
      rotateSecret({ db, kek }, jobId, { overlapHours: overlap }),
    ).rejects.toBeInstanceOf(InvalidOverlapError)
  })

  it('still creates a fresh secret when no prior active secrets exist', async () => {
    const { db, jobId } = await seedJob()
    const created = await rotateSecret({ db, kek, now: () => FIXED }, jobId, { overlapHours: 24 })
...
describe('revokeAllSecrets', () => {
  it('sets revoked_at on every non-revoked row', async () => {
    const { db, jobId } = await seedJob()
    await generateSecret({ db, kek, now: () => FIXED }, jobId)
    await generateSecret({ db, kek, now: () => new Date(FIXED.getTime() + 1000) }, jobId)
  ...
  it('leaves already-revoked rows untouched', async () => {
    const { db, jobId } = await seedJob()
    const { id: revokedId } = await generateSecret({ db, kek, now: () => FIXED }, jobId)
    const firstRevoke = new Date(FIXED.getTime() + 1000)
    await revokeAllSecrets({ db, now: () => firstRevoke }, jobId)

    await generateSecret({ db, kek, now: () => new Date(FIXED.getTime() + 2000) }, jobId)
  ...
  it('produces an active secret on generateSecret after revokeAllSecrets', async () => {
    ...
    const fresh = await generateSecret(
      { db, kek, now: () => new Date(FIXED.getTime() + 2000) },
      jobId,
    )
