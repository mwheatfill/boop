...
Populates a local D1 database with a fictional Arizona-MSP-credible dataset so the dashboard, palette, Workspace hub, and Job detail surfaces have something to render. Every demo row carries `seed_tag = 'demo-v1'`; cleanup deletes only those rows.
...
| Flag | Window | Workspaces | Jobs | Approx Run rows | Use |
|---|---|---|---|---|---|
...
- **Cleanup is scoped.** `--reset` deletes only rows where `seed_tag = 'demo-v1'` (or rows owned by those Workspaces/Operators). Operator-created rows with `seed_tag IS NULL` are never touched.
...
- **9 Workspaces** — Desert Vista Credit Union, Cactus Title, Sun Valley Insurance, Phoenix Healthcare Partners, Mesa Manufacturing Co., Salt River Logistics, Tempe Tech Group, Skyline Realty Trust, SwitchThink (internal). Mostly `America/Phoenix`; Salt River Logistics is `America/Los_Angeles`.
...
- **17 Channels** — Teams primary, email and webhook (used for PagerDuty/Autotask bridges) on critical Workspaces. Note: native `pagerduty` and `autotask` channel kinds are accepted by the DB schema but have no adapter, so the seed wires those integrations through the `webhook` channel kind.
...
If PRD #48 (`workspace_secrets`) or PRD #50 (`workspace_tunnels`) tables are not yet present, the corresponding seed modules are absent here and will land in follow-up PRDs.

## Operator data is never touched

`--reset` walks dependencies in FK order (attempts → runs → jobs → targets → workspaces; users last) using `WHERE seed_tag = 'demo-v1'` at the roots and inheriting via `workspace_id IN (<demo workspace ids>)` at the children. Real Workspace / Operator rows (where `seed_tag IS NULL`) are invisible to the cleanup query.
