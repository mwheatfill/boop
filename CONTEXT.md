...
**Target**:
A named, reusable HTTP destination owned by a Workspace, and the only tunnel-related object an Operator sees or manages. Carries method, authentication, and reachability. A **public** Target carries its own URL. A **private** Target (`reachability = 'tunnel'`) carries an **internal origin** (the on-prem `host:port` the Connector forwards to) and references one **Tunnel**; boop derives its public URL `https://<target-slug>.<tunnel-slug>.tunnels.<base>` and rebuilds the Tunnel's ingress to match. One Target is referenced by zero or more Jobs. Targets are picked-or-created inline from the Job form; Operators may create them.
_Avoid_: Endpoint (collides with API routes), Destination (collides with alert destinations), Hook URL

**Target health** (private Targets): a derived, per-Target state — **Operational**, **Origin unreachable** (Connector up but `cloudflared` can't reach the origin, e.g. Cloudflare 502 — IIS down or wrong port), **Tunnel offline** (the Target's Tunnel is not Operational), **Auth error** (Access rejected the Service Token), or **Checking** (no signal yet). Real Run outcomes override the synthetic probe.

**Tunnel**:
A Workspace-owned Cloudflare Tunnel: one **Connector** per customer **site**, fronting zero or more private Targets. boop provisions, operates, and decommissions it through the Cloudflare API (Model B: SwitchThink's Cloudflare account owns every Tunnel; the customer runs only the Connector install command). Each Tunnel owns a wildcard hostname `*.<slug>.tunnels.<base>` backed by a wildcard DNS record, a wildcard Access app (Service Token auth), and a wildcard advanced certificate; private Targets add ingress routes under it, so adding a Target needs no new DNS, Access app, or certificate. The internal origin lives on the **Target**, not the Tunnel. Operator-facing **state** is derived, never stored: **Provisioning** (boop creating Cloudflare resources / certificate issuing), **Install the connector** (resources ready, Connector not yet connected — the one operator action), **Operational** (Connector connected + certificate active), **Needs attention** (Connector degraded/down, or origins failing). UI surface: "Private Tunnels". Per [ADR-028](docs/adr/028-private-tunnels.md).
_Avoid_: Customer site, VPN, Proxy

**Connector**:
The `cloudflared` process the customer runs on a host inside their private network; it makes the outbound-only connection to Cloudflare that a Tunnel rides. One Connector per host (reuse a Tunnel by adding routes, not a Connector per Target); additional Connectors on other hosts are **replicas** for high availability (same Tunnel, same install token, no extra DNS/Access/certificate). Its Cloudflare status (`Healthy`, `Degraded`, `Down`, `Inactive`) drives the Tunnel state and reflects only the Connector-to-Cloudflare link, not whether the origin is reachable.
_Avoid_: Agent, Daemon, Tunnel (a Connector runs a Tunnel; they are not the same)

**Route**:
The Cloudflare ingress mapping `<target-hostname> → internal origin` on a Tunnel. Not a modeled entity in boop: it is derived from the active private Targets and rebuilt on every Target change, so it cannot drift. Operators act on Targets, never Routes; "route" stays an implementation word.
_Avoid_: surfacing "Route" in the UI
