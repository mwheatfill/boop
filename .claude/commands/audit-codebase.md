...
Files: `wrangler.jsonc`, `.github/workflows/*`, `package.json` scripts, `vite.config.ts` (if applicable).

Check against [Cloudflare Workers docs](https://developers.cloudflare.com/workers/) and [TanStack Start framework guide](https://developers.cloudflare.com/workers/framework-guides/web-apps/tanstack/) (use the `cloudflare-docs` MCP):
...
Check against [React 19 docs](https://react.dev) (use `context7` MCP for `react`):
...
Check via [shadcn llms.txt](https://ui.shadcn.com/llms.txt) and [shadcn docs](https://ui.shadcn.com/docs):
...
Files: `src/lib/db/*`, `src/lib/auth/*`, `drizzle/**`, `src/shared/schemas/auth.ts`, `src/shared/types/auth.ts`.

Check via [Drizzle docs](https://orm.drizzle.team/) and ADR-005 (auth abstraction):
...
- The Zod schema in `src/shared/schemas/auth.ts` flows into `openapi.json`; the type re-exported via `src/shared/types/auth.ts`.
...
Check against ADR-013 and [zod docs](https://zod.dev):
- Schemas imported from `@/shared/schemas/openapi` (loads `zod-openapi` side effect), not directly from `zod`.
...
Run an OWASP-relevant pass (no specific MCP; check against [OWASP Top 10](https://owasp.org/www-project-top-ten/) via WebFetch if needed):
...
- XSS: dynamic content rendered via React JSX (auto-escapes); flag any `dangerouslySetInnerHTML` without sanitization.
...
- No `eval`, no `new Function(...)`, no prototype mutation.

### 8. `code-quality` — anti-patterns, smells, naming, comments

Files: `src/**/*.ts(x)`, `scripts/**/*.ts`.

Check against [`AGENTS.md`](../../AGENTS.md) "File naming" + "Things to avoid" + general senior-engineer practice:
...
Files: `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `README.md`, `docs/adr/*`, `docs/roadmap.md`.
