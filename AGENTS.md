...
- **Don't ship UI without consulting `DESIGN.md` first.** Any change to `src/components/**`, `src/routes/**`, or `src/styles/**` cites the relevant `DESIGN.md` § in the PR body (e.g., "§ 3 Visual tokens" or "§ 9 Empty states"). The mechanical anti-patterns in `DESIGN.md` § 12 are caught by `pnpm audit:patterns`: Tailwind color classes with numeric scales (`text-blue-500`), arbitrary color values (`text-[#abc]`), and hardcoded colors in JSX attributes all fail CI. Use the token classes (`text-primary`, `bg-card`, `text-success/warning/info/destructive`, etc.) so colors flow from `--theme-base` / `--theme-accent`. See ADR-022.
