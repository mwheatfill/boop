...
**Flat, single-Workspace URLs** per [ADR-027](docs/adr/027-domain-simplification-workspace-consolidation.md) (superseding ADR-018's Customer grammar). v1 has one Workspace, resolved server-side in the `_authenticated` route context: there is no Workspace switcher and no Workspace segment in any URL. Navigation is Jobs-first.

```
/                                   Home (Jobs-first dashboard)
/jobs                               Jobs
/jobs/$jobSlug                      Job detail
/jobs/$jobSlug/runs/$runId          Run detail
/runs                               Run history
/templates                          Job templates
/targets                            Targets
/tunnels                            Private Tunnels (list + health)
/tunnels/$tunnelSlug                Tunnel detail
/channels                           Channels (admin)
/alert-rules                        Alert rules
/alert-rules/$ruleSlug              Alert rule detail
/recycle-bin                        Recycle bin (admin)
/w/$workspace/$slug                 Public webhook receiver
```

Detail surfaces (Job, Target, Channel, AlertRule, Tunnel) open as right-side Sheets from a row click or the top-right "New" action (see § 6), so most editing happens without a route change.

**Current layout: inverted-L sidebar.** A fixed-left sidebar at ~16rem (~256px) collapses to a ~3rem icon rail. Sections from top: brand mark, primary nav (Home, Jobs, Templates, Runs, Targets, Tunnels, Channels, Alert Rules), `Recent` (shared store with the Cmd+K palette), `Pinned` (device-local, capped at 20), footer with theme toggle + user menu. Below the `md` breakpoint the sidebar becomes a slide-in drawer via the shadcn `sidebar` primitive's built-in `Sheet` integration. Content area scrolls independently. The brand-vs-UI-accent split holds: chrome stays monochrome with the teal accent reserved for selection and primary action.

**Title strategy:**
- List views: small heading at top-left (`Jobs`, `Runs`, `Targets`). The data is the focus, not the page name.
- Detail views: large heading carrying the entity title (the Run, the Job, the Target).

**Breadcrumbs on detail pages** follow the entity nesting: `Jobs > db-backup > Run`.
...
**Primary action top-right.** "Create new Job," "Add Target," "Send test alert" land at the top-right corner of the content area, opposite the filters. Always the same hue: `--primary` (teal).
...
**Keyboard-first.** Every action reachable by mouse is reachable by keyboard. Tab order is logical and predictable. Focus is always visible: `--ring` is the teal `--primary`, a 2px ring with offset.
...
**Contrast.** Foreground / background contrast is in WCAG AA range for body text in both modes. Higher-contrast variants ship by swapping the flat token values (via the tweakcn export in § 3), not by a runtime derivation.
...
- **Re-introducing a derivation layer.** boop uses flat, pre-computed OKLCH tokens, not anchor-derived ones (e.g. `oklch(from var(--theme-base) calc(l + step) c h)`). Theme refreshes go through the tweakcn export workflow (§ 3).
...
- [ADR-018](docs/adr/018-navigation-ia.md): Navigation IA (URL grammar superseded by ADR-027; Jobs-first surfaces carried forward).
- [ADR-019](docs/adr/019-soft-delete-policy.md): Soft-delete policy (soft-delete with Recycle Bin restore/purge).
